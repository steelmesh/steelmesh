...
function _getPackageFolder(packageFile, callback) {
    var packageName = packageFile.replace(rePackageName, '$1'),
    	packageName = packageFile.substring(packageFile.lastIndexOf("\\")+1,packageFile.length);
		packageName = packageName.substring(0,packageName.indexOf("."));
	var packageFolder = path.join(packagesPath, packageName),
		versionMatch = reVersion.exec(packageFile),
        version = versionMatch ? versionMatch[2] : dateFormat(new Date(),"yyyy-mm-dd-HH-MM-ss"),
...
function _makePublisher(config, dash) {
    return function(req, res, next) {
        ...
        path.exists(appPath, function(exists) {
            if (exists) {
                app = new MeshApp(path.join(packagesPath, appid, version), {
                    id: appid
                });
