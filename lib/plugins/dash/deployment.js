...
function _getPackageFolder(packageFile, callback) {
    var packageFullName = path.basename(packageFile),
        packageName = packageFullName.substring(0,packageFullName.search(reValidFile)),
        packageVersion = dateFormat(new Date(),"yyyy-mm-dd-HH-MM-ss-l");

    // try catch version of package
    if ( packageName.search(reFileVersion) > 1 ) {
        packageVersion = packageName.substring(packageName.search(reFileVersion),packageName.length);
        packageName = packageName.replace(packageVersion,"");
        packageName = packageName.substring(0,packageName.length-1);
    }
    
    // check if exist version folder
    var packageFolder = path.join(packagesPath, packageName),
        versionFolder = path.join(packageFolder, 'v'+packageVersion);    
    path.exists(packageFolder, function(exists) {
        ...
        if ( os.platform() == "win32" || os.platform() == "win64" ) {
            exec("rmdir \"" + versionFolder + "\" /s /q", function(err) {
