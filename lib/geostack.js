...
Geostack = (function() {
    ...
    var extFiles = path.existsSync(baseConfig.pathExt) ? 
            fs.readdirSync(baseConfig.pathExt) : 
            [],
        extensions = [];
    
    for (var ii = 0; extFiles && ii < extFiles.length; ii++) {
        console.log('loading extension: ' + extFiles[ii]);
        extensions.push(require(path.join(baseConfig.pathExt, extFiles[ii])));
    } // for
    ...
    function details(geostack, callback, queryParams, req, res, next) { 
        var dsName = (req.params.dataset || '').replace(':', '_'),
            ...
            filePath = path.join(path.resolve(baseConfig.datapath || 'data'), fileId + '.json');
