    ...
    fs.readdir(config.pathExt, function(err, files) {
        for (ii = 0; (! err) && ii < files.length; ii++) {
            if (path.extname(files[ii]) == '.js') {
                ...
                if (autoLoad || config.enabledExtensions.indexOf(extensionName) >= 0) {
...
function NodeStack() {
    this.config = {
        ...
        // DEFAULT = null (all extensions will be loaded)
        // To control which extensions are loaded replace with an array of strings
