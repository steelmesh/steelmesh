...
NodeStack.prototype.detectMode = function(callback) {
    ...
    if (isWarChief) {
        warchief.init(this, function() {
            callback('warchief');
        });
    }
    else {
        // attempt to connect to the master
        var chiefHost = hostname.replace(reHost, this.config.warchief + '$2');
        
        this.log('attempting to connect the stack master: ' + chiefHost);
        warchief.connect(this, chiefHost, function(mode) {
            callback(mode);
        });
    } // if..else
}; // detectMode
...
NodeStack.prototype.init = function() {
    ...
    this.changeLogger = require('./changelogger');
    this.changeLogger.init(this);
    this.sendChange('village.online');
    ...
    this.detectMode(function(mode) {
        _this.log('stack mode detected, running as ' + mode + ' ' + (this.masterProcess ? 'master' : 'worker'));
        _this.mode = mode;
...
### sendChange
*/
NodeStack.prototype.sendChange = function(changeType, changeData) {
    if (this.changeLogger) {
