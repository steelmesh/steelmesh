...
module.exports = (function() {
    ...
    function detectMode(callback) {
        var hostname = os.hostname(),
            hostOnly = hostname.replace(reHost, '$1'),
            isMaster = hostOnly.toLowerCase() == config.master.toLowerCase();
            
        if (isMaster) {
            stackMaster.init(_this, function() {
                callback('master');
            });
        }
        else {
            // attempt to connect to the master
            var masterHost = hostname.replace(reHost, config.master + '$2');
            
            log('looking for ' + masterHost);
    ...
    detectMode(function(mode) {
        stackMode = mode;
        log('stack running in ' + mode + ' mode');
    });
