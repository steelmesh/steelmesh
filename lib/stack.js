...
function NodeStack() {
    ...
    // initialise the warchief hostname
    this.warchiefHost = undefined;
...
NodeStack.prototype.cleanup = function() {
    ...
    // if we have a changelogger ask it to shutdown
    if (this.changeLogger) {
        this.changeLogger.shutdown(this);
    } // if
...
NodeStack.prototype.createServer = function(logExceptions) {
    ...
    return connect.createServer(
        ...
        connect.router(function(app) {
            ...
            // when the mode is updated check to see if we should route connections
            // if we are the highlander (the warchiefs only villager, then yes)
            _this.on('updateMode', function(mode) {
                if (mode == 'highlander') {
                    require('./warchief').router(app, _this);
                } // if
            });
        })
    );
...
NodeStack.prototype.detectMode = function(callback) {
    var hostname = os.hostname(),
        ...
        isWarChief = hostOnly.toLowerCase() == this.config.warchief.toLowerCase();
    ...
    if (isWarChief) {
        this.mode = this.masterProcess ? 'warchief' : 'highlander';
    }
    // otherwise, the master process is the chief and worker processes are villagers
    else {
        this.mode = this.masterProcess ? 'chief' : 'villager';
        this.warchiefHost = this.config.warchief + hostname.replace(reHost, '$2');
    } // if..else
...
NodeStack.prototype.init = function() {
    ...
    this.on('updateMode', function() {
        _this.cluster.set('workers', _this.isWarChief() || !os ? 1 : os.cpus().length);

        // if the change logger has been created, then shut it down
        if (_this.changeLogger) {
            _this.changeLogger.shutdown(_this);
            _this.changeLogger = null;
        } // if..else

        // create the change logger
        _this.changeLogger = require('./changelogger');
        _this.changeLogger.init(_this);
        _this.log('loaded change logger');
    });
...
NodeStack.prototype.isWarChief = function() {
    return this.mode === 'warchief';
}; // isWarChief
