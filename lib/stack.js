...
NodeStack.prototype.settingWrite = function(section, name, value, callback) {
    loadSettingsFile(this, function() {
        if (! _this.settings[section]) {
            _this.settings[section] = {};
        } // if

        // update the setting value
        _this.settings[section][name] = value;

        // write the settings file
        fs.writeFile('settings.json', JSON.stringify(_this.settings), 'utf8', callback);
    });
}; // settingWrite
