...
function _loadApp(mesh, appData, appUrl, callback) {
    var appPath, app;
    
    // ensure we have an app id
    appData.id = appData.id || appData._id.slice(_appPrefix.length);
    appPath = path.resolve(__dirname, '../../apps/' + appData.id);
    mesh.log.debug('loading app: ' + appData.id);
        
    // if the appData value contains a mount point, then map the app
    if (appData.mountpoint) {
        mappedApps[appData.mountpoint] = appData.id;
    }
    ...
    app.on('load', function() {
        ...
        // if the application url has been specified, then download the application
        if (appUrl) {
            mesh.log.info('updating app from: ' + appUrl);
            attachmate.download(
                appUrl,
                appPath,
                function(err) {
                    if (err) {
                        mesh.log.error('Error downloading app "' + appData.id + '"', err);
                    }
                    else {
                        mesh.log.info('Completed downloading app: ' + appData.id);
                    }
...
CouchAppLoader.prototype.loadApps = function(mesh, callback, downloadApps) {
    ...
    db.list(function(err, res) {
        if (err) {
            ...
            // if we have no rows, then trigger the callback
            if (rows.length === 0) {
                callback(null, apps);
            }
            else {
                ...
                _.each(rows, function(row) {
                    db.get(row.id, function(err, res) {
                        if (! err) {
                            // delete the attachments from the data
                            delete res._attachments;

                            _loadApp(mesh, res, downloadApps ? (dbUrl + '/' + row.id) : null, function(app) {
                                if (mesh.messenger && cluster.isMaster) {
                                    mesh.messenger.send('app', app.toJSON());
                                }

                                // update the apps array
                                apps.push(app);
                                if (apps.length >= rows.length) {
                                    callback(null, apps);
                                }
                            });
                        }
...
CouchAppLoader.prototype.publish = function(config, app, callback) {
    var couchurl = config.admin.couchurl || config.couchurl,
        ...
        opts = {
            docData: app.toJSON(true), // get the json data, dropping persistent properties
            preserveExisting: false
        };
        
    // upload the files to couch
