var async = require('async'),
    ...
    _healthCheckTimer = 0,
    _reconnectDelay = (config.monitor.reconnectDelay || 5) * 1000,
...
function _monitorSteelmesh() {
    ...
    log.debug('monitoring steelmesh @ ' + targetUrl);
    
    // monitor steelmesh for changes
    changemate(targetUrl, opts, function(err, notifier) {
        if (! err) {
            log.debug('monitoring steelmesh db for changes');
            ...
            notifier.on('close', function() {
                log.debug('lost connection to steelmesh db, will attempt to reconnect');
