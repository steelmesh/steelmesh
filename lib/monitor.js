...
function _monitorSteelmesh() {
    ...
    changeling(config.couchurl.replace(reTrailingSlash, '') + '/' + config.meshdb, opts, function(err, notifier) {
        if (! err) {
...
function _watchForChanges(app, dbUrls) {
    ...
    function attachMonitor(target, dbid, dbConfig) {
        ...
        changeling(target, { type: 'couchdb', getDoc: true }, function(err, notifier) {
            if (err) {
