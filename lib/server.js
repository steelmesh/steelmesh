...
SteelmeshServer.prototype.start = function(callback) {
    ...
    */
    ...
    this.initMonitor(logger);
    
    // initialise the apploader
    this.log.info('loading apps using the !{underline}{0}!{} apploader', config.apploader);
    apploader.loadApps(mesh, function(apps) {
        mesh.apps = apps || [];

        // master initialization
        if (cluster.isMaster) {
            mesh.log.info('Steelmesh master process initialized, forking {0} workers', os.cpus().length);
