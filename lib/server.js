...
SteelmeshServer.prototype.start = function(opts, callback) {
    ...
    // if we don't have a monitor process, then create it
    if (cluster.isMaster && (opts.restartMonitor || (! this.monitor))) {
        // if we have an existing monitor, then kill that process
        if (this.monitor) {
            log.info('terminating existing monitor process (pid = ' + this.monitor.pid + ')');
            this.monitor.kill();
        }
        
        log.info('starting monitor process');
        this.monitor = _fork('monitor', this.messenger, function() {
            log.info('steelmesh monitor process running');
        });
    }
...
exports.start = function() {
    ...
    require('./helpers/messaging').create(function(messenger) {
        ...
        if (cluster.isMaster) {
            log.debug('starting the dashboard process');
