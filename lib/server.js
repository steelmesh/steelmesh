...
function _masterOnly(targetFunction) {
    if (cluster.isMaster) {
        targetFunction.apply(_meshServer, Array.prototype.slice.call(arguments, 1));
    }
    else {
...
SteelmeshServer.prototype.start = function(opts, callback) {
    ...
    function realStart() {
        async.series([
            ...
            function(itemCallback) {
                _masterOnly(mesh.discover, itemCallback);
            }
        ], function(err) {
            if (err) {
                mesh.terminate(err);
    ...
    // start the dashboard
    _masterOnly(this.subprocessStart, 'dashboard', opts, function() {
        // ensure the monitor is running
        _masterOnly(mesh.subprocessStart, 'monitor', opts, function() {
...
SteelmeshServer.prototype.subprocessStart = function(name, restartOpts, callback) {
    var mesh = this;
