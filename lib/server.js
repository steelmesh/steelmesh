...
function _fork(target) {
    return cp.fork(path.resolve(__dirname, target), null, {
...
function _processAction(msg) {
    if (msg && msg.action) {
        if (_meshServer.log) {
            _meshServer.log.info('Captured action: ' + msg.action);
        }
        
        try {
            require('./helpers/actions/' + msg.action).call(null, _meshServer, msg);
        }
        catch (e) {
            if (_meshServer.log) {
                _meshServer.log.error('Could not load message handler for action: ' + msg.action, e);
            }
        }
    }
} // _processMessage
...
exports.start = function() {
    ...
    require('./helpers/messaging').create(function(messenger) {
        ...
        if (cluster.isMaster) {
            ...
            _meshServer.dashboard = _fork('dashboard.js');

            // capture the dash-ready event
            messenger.on('dash-ready', function() {
