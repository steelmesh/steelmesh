...
exports.init = function(params) {
    ...
    */
    ...
    ### cql(queryParams)
    */
    function cql(stack, callback, queryParams, req, res) {
        getActiveServer(function(url) {
            ...
            request({ uri: targetUri }, function(err, response, body) {
                if (! err) {
                    ...
                    stack.reportError(callback, 'Invalid response from geoserver: ' + err);
                }
            });
        });
    } // cql
    
    function filter(stack, callback, queryParams, req, res) {
        var wfsRequest,
            log = stack.initLog();
        ...
        if (wfsRequest) {
            ...
            stack.reportError(callback, 'Unable to locate required filter or conditions parameter');
        } // if..else
    } // filter    
