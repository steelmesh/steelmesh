...
function _loadConfig(app, appPath, callback) {
    ...
    async.parallel([
        function(taskCb) {
            path.exists(appDefinition, function(exists) {
                if (exists) {
        ...
        function(taskCb) {
            ...
            fs.readFile(appPackage, 'utf8', function(err, data) {
                if (! err) {
                    app.packageData = JSON.parse(data);
                }
            });
...
var MeshApp = exports.MeshApp = function(appPath, config) {
    ...
    _loadConfig(this, appPath, function() {
        // override loaded configuration with config passed to the constructor
        _.extend(app, config);
...
MeshApp.prototype.toJSON = function() {
    return {
        id: this.id, 
        title: this.title,
        path: this.basePath,
