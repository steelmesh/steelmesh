...
module.exports = (function() {
        ...
        fs.readdir('lib/couchapps', function(err, files) {
            ...
            for (var ii = 0; ii < files.length; ii++) {
                if (reValidCouchApp.test(files[ii])) {
                    loadCouchApp(stack, couchUrl, path.basename(files[ii], '.js'));
                } // if
            } // for
        });
    } // findCouchApps
    ...
    function checkDatasets(stack, callback) {
        
        function checkDS(ds, callback) {
            queryTypeDesign(stack, ds.key, {}, function(res) {
                if (! res.error ) {
                    stack.validateDesign(ds.key, res);
                } // if 
            });
        } // validateDataset
        ...
        couch.queryDesign(datasetsQuery, function(res) {
            ...
            if (res.rows) {
                async.forEach(res.rows, checkDS, function(err, results) {
                    console.log('checked');
                    callback();
                });
            }
