...
module.exports = (function() {
    ...
    function dbReset(mesh, callback) {
        mesh.log.debug('CouchDB reset detected, resetting lastChangeId setting');
    ...
    function init(mesh) {
        couch = nano(mesh.config.couchurl).use(mesh.config.datadb);
            ...
            runStartupChecks(mesh, function() {
                ...
                couch.get(function(error, res) {
                    if (! error) {
                        ...
                        mesh.log.warn('No connection to CouchDB');
                    } // if..else
                });
            });
        }
