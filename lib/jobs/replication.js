    ...
    if (mesh.isSlave() && mesh.masterHost) {
        if (mesh.couch && !replicationActive) {
            ...
            mesh.couch.post({
                ...
                if (error) {
                    mesh.log.error('replication error from ' + mesh.masterHost + ': ' + error);
                } // if
                else if (res.ok && !res.no_changes) {
                    mesh.log.debug('replication session ' + res.session_id + ' complete');
                    
                    mesh.settingRead('couch', 'lastChangeId', function(lastChangeId) {
                        if (lastChangeId > res.source_last_seq) {
                            mesh.log.info('CouchDB reset detected, resetting lastChangeId setting');
